...
def test_insert_extension(db, books, ir_model_data):
    ...
    table_name = 'books'
    header = 'title[unique=true], authorid(name), bookid(name)[table=ir_model_data: id=bookid: target-name=res_id: qualifier=netsuite_books]'
    body = '''
...
def test_insert_unique_extension(db, books, ir_model_data, context):
    ...
    table_name = 'books'
    header = 'title, authorid(name), bookid(name)[table=ir_model_data: id=bookid: target-name=res_id: qualifier=netsuite_books: unique=true]'
    body = '''
        Pride and Prejudice, Jane Austen, 12345 
    '''
    # post
